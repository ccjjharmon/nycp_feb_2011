...
namespace nothinbutdotnetprep.infrastructure.filtering
{
                                                                           ...
                                                                           : IComparable<PropertyType>
    {
        ...
        {
            return original.execute(condition);
        }
        ...
        {
            return execute(x => accessor(x).CompareTo(value) > 0);
        ...
        public Criteria<ItemToFilter> between(PropertyType start,
                                              PropertyType end)
        {
            return execute(x => accessor(x).CompareTo(start) >= 0 && accessor(x).CompareTo(end) <= 0);
